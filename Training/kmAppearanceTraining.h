...
namespace km
{
		...
		const char* outputdir)
	{
		...
		for (unsigned int i=0;i<numberOfData;i++)
		{
			...
			km::ProfileExtractor<OriginalImageType, SimplexMeshType> profileExtractor;
			profileExtractor.setImage(origimage);
			profileExtractor.enableCache(false);
			profileExtractor.extractGlobalIntensityRange(livermesh);
			//profileExtractor.extractLocalIntensityRange(livermesh);
			...
			while (geoIt!=geoItEnd)
			{
				...
				for (int t=1;t<=g_number_of_inside_per_point;t++){
					points_inside.push_back(ipoint-normal*g_shift_inside*t);
				}
				for (int t=0;t<g_number_of_boundary_per_point;t++){
					points_boundary.push_back(ipoint+normal*((rand()%10-5.0)/10.0)*g_shift_boundary);
				}
				for (int t=1;t<=g_number_of_outside_per_point;t++){
					points_outside.push_back(ipoint+normal*g_shift_outside*t);
				}

				std::vector<FLOATTYPE> features;
				//提取向法线内部方向shift后的位置的Profile

				//KM_DEBUG_INFO("Extract inside profiles...");
				for (int t=0;t<points_inside.size();t++)
				{
					PointType ipoint_inside = points_inside[t];

					//Liver
					profileExtractor.extractFeatureSet(features, profileUnitLiver.category, geodata, ipoint_inside, idx);
					profileUnitLiver.writeLine(idx, IPClass, features);

					//Boundary
					profileExtractor.extractFeatureSet(features, profileUnitBoundary.category, geodata, ipoint_inside, idx);
					profileUnitBoundary.writeLine(idx, IPClass, features);

					//Plain
					if (t<1)
					{
						profileExtractor.extractFeatureSet(features, profileUnitPlain.category, geodata, ipoint_inside, idx);
				...
				//提取当前位置的Profile
				//KM_DEBUG_INFO("Extract boundary profiles...");
				for (int t=0;t<points_boundary.size();t++)
				{
					PointType ipoint_boundary = points_boundary[t];

					//Boundary
					profileExtractor.extractFeatureSet(features, profileUnitBoundary.category, geodata, ipoint_boundary, idx);
					profileUnitBoundary.writeLine(idx, BPClass, features);

					//Plain
					if (t<1)
					{
						profileExtractor.extractFeatureSet(features, profileUnitPlain.category, geodata, ipoint_boundary, idx);
						profileUnitPlain.writeLine(idx, BPClass, features);
					}
				}

				//提取向法线外部方向shift后的位置的Profile
				//KM_DEBUG_INFO("Extract outside profiles...");
				for (int t=0;t<points_outside.size();t++)
				{
					...
					//Liver
					profileExtractor.extractFeatureSet(features, profileUnitLiver.category, geodata, ipoint_outside, idx);
					profileUnitLiver.writeLine(idx, OPClass, features);

					//Plain
					if (t<1)
					{
						profileExtractor.extractFeatureSet(features, profileUnitPlain.category, geodata, ipoint_outside, idx);
