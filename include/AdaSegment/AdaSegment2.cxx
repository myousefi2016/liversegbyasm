...
int main( int argc, char ** argv )
{
	if ( argc < 5 )
	{
		...
		std::cerr << argv[0]
		<< " inputImage abdominalMaskImage AdaBoostOutPutPrefix outputSegmentation"<<endl<<endl
	...
	abdominalmaskSeg->SetFileName(argv[2]);
	try
	{
	...
	UCIndexIteratorType abdominalmask_it(abdominalmaskSeg->GetOutput(),abdominalmaskSeg->GetOutput()->GetLargestPossibleRegion());
	...
	UCLineIteratorType abdominallineit(abdominalmaskSeg->GetOutput(),abdominalmaskSeg->GetOutput()->GetLargestPossibleRegion());
	...
	UCImageType::SpacingType spacing = abdominalmaskSeg->GetOutput()->GetSpacing();
	...
	UCImageWriteIteratorType nsegit(nseg, nseg->GetRequestedRegion());
