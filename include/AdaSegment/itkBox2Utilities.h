 ...
 *
 *=========================================================================*/
#ifndef __itkBox2Utilities_h
#define __itkBox2Utilities_h
...
TIterator *
setConnectivityEarlyBox2(TIterator *it, bool fullyConnected = false)
{
...
void
Box2AccumulateFunction(const TInputImage *inputImage,
                      ...
                      ProgressReporter & progress)
{
  ...
  setConnectivityEarlyBox2(&noutIt, true);
...
std::vector< typename ImType::OffsetType > CornerOffsets2(const ImType *im)
{
...
template< class TInputImage, class TOutputImage >
void
Box2MeanCalculatorFunction(const TInputImage *accImage,
                          ...
                          ProgressReporter & progress)
{
  ...
  std::vector< OffsetType > UnitCorners = CornerOffsets2< TInputImage >(accImage);
...
template< class TInputImage, class TOutputImage >
void
Box2SumCalculatorFunction(const TInputImage *accImage,
						  TOutputImage *outputImage,
						  typename TInputImage::RegionType inputRegion,
						  typename TOutputImage::RegionType outputRegion,
						  typename TInputImage::SizeType Radius,
	...
	for ( fit = faceList.begin(); fit != faceList.end(); ++fit )
	{
		if ( fit == faceList.begin() )
		{
			...
			for ( oIt.GoToBegin(); !oIt.IsAtEnd(); ++oIt )
			{
				...
				oIt.Set( static_cast< OutputPixelType >( Sum ) );  //sum
				progress.CompletedPixel();
			}
		}
		else
...
template< class TInputImage, class TOutputImage >
void
Box2SigmaCalculatorFunction(const TInputImage *accImage,
                           TOutputImage *outputImage,
                           typename TInputImage::RegionType inputRegion,
                           typename TOutputImage::RegionType outputRegion,
                           typename TInputImage::SizeType Radius,
...
template< class TInputImage, class TOutputImage >
void
Box2SquareAccumulateFunction(const TInputImage *inputImage,
                            TOutputImage *outputImage,
                            typename TInputImage::RegionType inputRegion,
                            typename TOutputImage::RegionType outputRegion,
                            ProgressReporter & progress)
