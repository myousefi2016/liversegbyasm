...
namespace itk
{
    ...
    ::SetProbabilityImage( ProbabilityImagePointer pImage )
  {
    ...
    this->m_UseProbability = true;

    m_ProbabilityInterpolator = FloatInterpolatorType::New();
    m_ProbabilityInterpolator->SetInputImage( this->m_ProbabilityImage );
  }
    ...
    ::RemoveProbabilityImage(  )
  {
    ...
    this->m_UseProbability = false;

    m_ProbabilityInterpolator = NULL;
  }
	...
	*/
		...
		throw ( ExceptionObject )
	{
		...
		*/
		...
		if(m_ProbabilityImage)
		{
			m_UseProbability = true;
			//std::cout<<"With probability image!!"<<std::endl;

			//m_MovingLabelImage = km::binaryThresholdImage<MovingImageType, MovingImageType>( this->GetMovingImage(), 50, 300, 1, 0 );
		}
		else
		{
			m_UseProbability = false;
			//std::cout<<"Without probability image!!"<<std::endl;
		}
	...
	*/
		...
		movingImageGradientValue) const
	{
		...
		*/
		...
		if( m_UseProbability )
		{
			FixedImagePointType fixedPoint = this->m_FixedImageSamples[fixedImageSample].point;
			double fixedValue = this->m_FixedImageSamples[fixedImageSample].value;

			if( m_ProbabilityInterpolator->IsInsideBuffer( fixedPoint ) )
			{
				probabilityValue = m_ProbabilityInterpolator->Evaluate( fixedPoint );
			}

			//probabilityValue = std::sqrt( probabilityValue );
		}
		...
		this->m_ThreaderFixedImageMarginalPDF[threadID][fixedImageParzenWindowIndex] += 1*probabilityValue;
		...
		*/
		...
		while( pdfMovingIndex <= pdfMovingIndexMax )
		{
			...
			PDFValueType pdfValue = static_cast<PDFValueType>( this->m_CubicBSplineKernel ->Evaluate( movingImageParzenWindowArg) );
			pdfValue *= probabilityValue;
			...
			*( pdfPtr++ ) += pdfValue;
			...
			if( this->m_UseExplicitPDFDerivatives || this->m_ImplicitDerivativesSecondPass )
			{
				...
				PDFValueType cubicBSplineDerivativeValue = this->m_CubicBSplineDerivativeKernel->Evaluate(movingImageParzenWindowArg);
	...
	*/
		...
		PDFValueType cubicBSplineDerivativeValue) const
	{
		...
		PDFValueType precomputedWeight = 1.0;
  ...
  /**
	...
	*/
    ...
    const double weightFactor) const
	{
		// Update bins in the PDF derivatives for the current intensity pair
		// Could pre-compute

		PDFValueType precomputedWeight = weightFactor;
		...
		//std::cout<<precomputedWeight<<std::endl;
		...
		if( !this->m_TransformIsBSpline )
		{
			...
			*/
			...
			for( unsigned int mu = 0; mu < this->m_NumberOfParameters; mu++ )
			{
				...
				const PDFValueType derivativeContribution = innerProduct * cubicBSplineDerivativeValue * precomputedWeight;
