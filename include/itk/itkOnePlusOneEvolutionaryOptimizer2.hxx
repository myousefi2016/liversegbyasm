 ...
 *
 *=========================================================================*/
#ifndef _itkOnePlusOneEvolutionaryOptimizer2_hxx
#define _itkOnePlusOneEvolutionaryOptimizer2_hxx

#include "itkOnePlusOneEvolutionaryOptimizer2.h"
  ...
  for ( unsigned int iter = 0; iter < m_MaximumIteration; iter++ )
    ...
    if ( m_Maximize )
      ...
      if ( cvalue < pvalue )
        ...
        for ( unsigned int i = 0; i < spaceDimension; i++ )
          {
						if(m_UseNumberOfCoefficient)
						{
							if(i>=m_UseNumberOfCoefficient || std::abs(parent[i])>=3)
							{
								parent[i]=0;
							}
						}
