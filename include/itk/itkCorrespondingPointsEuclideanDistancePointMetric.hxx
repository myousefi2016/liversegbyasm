...
::GetValue( const TransformParametersType & parameters ) const
{
  ...
  while( pointItFixed != pointEnd )
  {
    ...
    mappedPoint = this->m_Transform->TransformPoint( movingPoint );
    ...
    if( sampleOk )
    {
      ...
      VnlVectorType diffPoint = ( fixedPoint - mappedPoint ).GetVnlVector();
  ...
  MeasureType & value, DerivativeType & derivative ) const
{
  ...
  while( pointItFixed != pointEnd )
  {
    ...
    if( sampleOk )
    {
	  ...
	  this->m_Transform->ComputeJacobianWithRespectToParameters( movingPoint, jacobian );
