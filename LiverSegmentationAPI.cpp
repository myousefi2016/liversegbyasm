			  ...
			  const char* varianceMapFile)
{
	...
	notifier->notifyMesh( deformedMesh, "locatedMesh.vtk" );
	...
	if(flag_fittingByDistMap)
	{
		...
		notifier->notifyMesh( deformedMesh, "shapeFittingMeshByDistMap.vtk" );
	}
	...
	if (flag_deformingByLiverProfile)
	{
		...
		while ( flagLooping )
		{
			...
			generateBestPointSet<FloatImageType, MeshType, GradientInterpolatorType, IntensityInterpolatorType>
				( &adaboostProfileClassifier_Liver, gradientInterpolator, intensityInterpolator, bestMesh, shapeMesh, varianceMap, 2.0, 20);
		...
		notifier->notifyMesh(shapeMesh, "LIVER-shapeMesh.vtk");
		notifier->notifyMesh(bestMesh, "LIVER-bestMesh.vtk");
	}
