...
int main(int argc, char * argv[] )
{
  ...
  km::writeImage<OriginalImageType>("originalImage.nii.gz", originalImage);
  ...
  std::cout << "Sigmoid is DONE!" << std::endl;
	km::writeImage<OriginalImageType>("sigmoidImage.nii.gz", sigmoidimagefilter->GetOutput());
	*/
	TriangleMeshType::Pointer trangleMesh = km::readMesh<TriangleMeshType>(argv[1]);
	SimplexMeshType::Pointer  simplexMesh = km::triangleMeshToSimplexMesh<TriangleMeshType,SimplexMeshType>(trangleMesh);

	OriginalImageType::Pointer sigmoidImage = km::readImage<OriginalImageType>(argv[2]);

  //GradientFilterType::Pointer gradientFilter = GradientFilterType::New();
  //gradientFilter->SetInput( sigmoidImage );
  //gradientFilter->SetSigma(1.0);
  //gradientFilter->Update();
  //std::cout << "GradientMagnitude is DONE!" << std::endl;
	GradientImageType::Pointer gradientImage = km::calculateRecursiveGradientImage<OriginalImageType,GradientImageType>(sigmoidImage,5);
	km::writeImage<GradientImageType>("gradientImage.mha", gradientImage);
	//gradientImage = km::calcuateGVF<GradientImageType>(gradientImage, 5, 1000);
	//km::writeImage<GradientImageType>("gradientImageGVF.mha", gradientImage);
  ...
  for (unsigned int i=0 ; i < numberOfCycles; i++)
    ...
    deformFilter->SetAlpha(atof(argv[3])); //0.1
    deformFilter->SetBeta(atof(argv[4])); //-0.1
    deformFilter->SetIterations(atoi(argv[5])); //100
