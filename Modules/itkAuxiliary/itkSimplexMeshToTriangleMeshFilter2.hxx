...
::Initialize()
{
  SimplexPolygonVisitorInterfacePointer simplexVisitor = SimplexPolygonVisitorInterfaceType::New();
  SimplexTriangleVisitorInterfacePointer simplexTriangleVisitor = SimplexTriangleVisitorInterfaceType::New();

  simplexVisitor->SetMesh( this->GetInput(0) );
  simplexTriangleVisitor->SetMesh( this->GetInput(0) );

  CellMultiVisitorPointer mv = CellMultiVisitorType::New();
  mv->AddVisitor(simplexVisitor);
  mv->AddVisitor(simplexTriangleVisitor);
  this->GetInput(0)->Accept(mv);
  this->GetInput(0)->BuildCellLinks();
  m_Centers = simplexVisitor->GetCenterMap();
  PointMapPointer centers2 = simplexTriangleVisitor->GetCenterMap();

  //std::cout<<m_Centers->Size()<<std::endl;
  //std::cout<<centers2->Size()<<std::endl;

  PointMapType::ConstIterator mapIt = centers2->Begin();
  while (mapIt != centers2->End())
  {
    m_Centers->InsertElement( mapIt->Index(), mapIt->Value() );
	mapIt++;
  }
}
...
::CreateTriangles()
{
  ...
  while ( pointsIt != points->End() )
    ...
    if ( !( b1 && b2 && b3 ) )
    {
	  ...
	  std::cout<<"Warning: Assertion failed for test of GetElementIfIndexExists()"<<std::endl;
    }
	else
	{
