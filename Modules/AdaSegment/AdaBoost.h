...
void weakClassifier(node *X, int NSample, weakLearner &wl){
    ...
    FLOATTYPE p1=0,p2,mp;
    unsigned long long int i;//, NSample=X.size();
    for (i=0;i<NSample;i++)
	{
        if (X[i].y==1)
           p1+=X[i].w;
...
{
    const unsigned long long mn1 = (last - first - 1);
    const unsigned long long n   = (last - first) / m;
    std::vector<bool> visited(last - first);
    RandomIterator cycle = first;
    while (++cycle != last) {
        if (visited[cycle - first])
            continue;
        unsigned long long a = cycle - first;
        do  {
            a = a == mn1 ? mn1 : (n * a) % mn1;
...
int AdaBoostTrain(FLOATTYPE* X, char* Y,unsigned long long int NSample, int NFeature, int iterN, char* fileName){
    ...
    FLOATTYPE *W= new FLOATTYPE[NSample];

	FLOATTYPE tw=1.0/NSample, R;

    char *CH = new char[NSample];
	char *F  = new char[NSample];
    FLOATTYPE *H  = new FLOATTYPE[NSample];
    node * tX=new node[NSample];
    FLOATTYPE weightedError, alpha, totalW;
    FLOATTYPE trueError;
    unsigned long long int i,j,CC=0;
    unsigned long long int LP=0, LN=0;
    for (i=0;i<NSample;i++)
	{
        if (Y[i]==1)
            LP++;
    ...
    LN=NSample-LP;
    if (LP>LN)
        R = FLOATTYPE(LN)/NSample;
    else
        R = FLOATTYPE(LP)/NSample;
    ...
    typedef std::pair<FLOATTYPE, int> sort_index_pair;
    ...
    while (CC<iterN)
	{
        ...
        FLOATTYPE *bwl_xpos = X;

        for (i=0;i<NFeature;i++)
		{
            
            // Pointer to all the samples for the i-th feature
            FLOATTYPE *xpos = X + i * NSample;

            for (j=0;j<NSample;j++)
			{
                unsigned long long int si = *ipos++;
                tX[j].x = xpos[si];
                tX[j].y = Y[si];
                tX[j].w = W[si];
            }
            ...
            if (wl.weightedRate>bwl.weightedRate)
			{
               ...
               bwl.featureID=i;
               bwl_xpos = xpos;
			 //  std::cout<<"bwl.weightedRate: "<<bwl.weightedRate<<std::endl;
            }
        }
        alpha = 0.5 * log(bwl.weightedRate/(1.0-bwl.weightedRate));
        weightedError=1.0;
        totalW=0;
        trueError=0;
        for (i=0;i<NSample;i++)
		{
            if (bwl.sign==1)
                CH[i] = 2*(bwl_xpos[i]>bwl.threshold) - 1;
            else
                CH[i] = 2*(bwl_xpos[i]<=bwl.threshold) - 1;
            F[i] = char(CH[i]==Y[i]);
            H[i] += alpha*double(CH[i]);
            if (double(H[i])*double(Y[i])<0)
                trueError ++;
            weightedError -= double(F[i])*W[i];
...
void AdaBoostClassify(FLOATTYPE *X, int NSample, int NFeature, int LC, int *featureID, FLOATTYPE *alpha, int *sign, FLOATTYPE *threshold, FLOATTYPE* H, FLOATTYPE* cH){
    int i,j;
